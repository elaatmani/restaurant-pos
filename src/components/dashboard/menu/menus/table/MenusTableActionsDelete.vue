    ...
    <div>
        ...
        <popup :closeable="!loading" :visible="visible" @cancel="visible = false">
            <div
                ...
                <h1 class="tw-text-lg tw-font-semibold tw-p-5 tw-bg-gray-100">Vous êtes sûr de vouloir supprimer cette menu ?</h1>
...
const destroy = async () => {
    ...
    await Menu.delete(props.data.id)
    .then(
        res => {
            if(res.data.status == 200) {
                store.deleteMenu(props.data.id);
                useAlert('Menu a été supprimé');
                visible.value = false;
            }
        },
        err => {
